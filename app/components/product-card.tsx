...
import { Star, ShoppingCart, ChevronLeft, ChevronRight, Zap } from "lucide-react"
    ...
    >
      ...
      <CardContent className="p-4">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="font-bold text-lg">{product.price} DH</span>
        </div>
        ...
        <div className="space-y-2">
          ...
          >
            <ShoppingCart className={`h-4 w-4 mr-1 sm:mr-2 ${isAddingToCart ? "animate-bounce" : ""}`} />
            {isAddingToCart ? "Ajout..." : <span className="hidden sm:inline">Ajouter au Panier</span>}
            {isAddingToCart ? "" : <span className="sm:hidden">Panier</span>}
          </Button>
          ...
          >
            <Zap className={`h-4 w-4 mr-1 sm:mr-2 ${isBuyingNow ? "animate-spin" : ""}`} />
            {isBuyingNow ? "Redirection..." : <span className="hidden sm:inline">Acheter</span>}
            {isBuyingNow ? "" : <span className="sm:hidden">Acheter</span>}
          </Button>
        </div>
      </CardContent>
    </Card>
